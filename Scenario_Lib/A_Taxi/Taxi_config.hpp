...
#ifndef TAXI_CONFIG_H
#define TAXI_CONFIG_H
...
namespace TaxiConfig {
     ...
     * @param filename 配置文件名，默认为 "taxi_config.txt"
     * @note 支持注释和空行，参数格式为 key = value
     */
    void loadConfig(const std::string& filename = "taxi_config.txt") {
        std::ifstream file(filename);
        if (!file.is_open()) {
            std::cout << "[TaxiConfig] 配置文件不存在，使用默认值" << std::endl;
            return;
        }
        ...
        while (std::getline(file, line)) {
            ...
            if (equal_pos == std::string::npos) {
                std::cout << "[TaxiConfig] 警告：第" << line_count << "行格式错误，缺少等号: " << line << std::endl;
                continue;
            }
            ...
            try {
                ...
                std::cout << "[TaxiConfig] 警告：第" << line_count << "行数值格式错误: " << value_str << std::endl;
                continue;
            }
            ...
            if (key == "MAX_THROTTLE") {
                MAX_THROTTLE = value;
                std::cout << "[TaxiConfig] 加载 " << key << " = " << value << std::endl;
            }
                ...
                std::cout << "[TaxiConfig] 警告：未知参数 " << key << std::endl;
            }
        }
        
        std::cout << "[TaxiConfig] 配置文件加载完成" << std::endl;
    }
