...
#include "abort_takeoff_config.hpp"          // 本科目头文件, 配置文件，参数定义
#include "abort_takeoff_events.hpp"          // 本科目头文件, 事件定义，事件枚举与条件
#include "abort_takeoff_initial_state.hpp"   // 本科目头文件, 初始状态，仿真初始值
...
int main() {
    ...
    // 设置控制台编码为UTF-8，解决中文输出乱码问题
    SetConsoleOutputCP(CP_UTF8);
    SetConsoleCP(CP_UTF8);

    // 加载中止起飞配置文件，你可以通过修改（手动修改或者编程修改）配置文件，对同一场景下的不同影响场景参数的影响分析，这为批量化仿真（态势感知）能力提供了基础
    std::cout << "[主函数] 开始加载配置文件..." << std::endl;
    AbortTakeoffConfig::loadConfig("abort_takeoff_config.txt");
    ...
    // 创建并初始化共享状态空间：共享状态空间，在ParaSAFE框架下特别重要。它包含了现代控制理论中所述的动态系统的状态空间，飞行器及其系统的特征参数，仿真控制所需的特征参数
    SharedStateSpace state; // 创建共享状态空间
    AbortTakeoffInitialState::initializeMotionState(state, aircraftConfig); // 初始化共享状态空间
    state.simulation_started = true;   // 设置仿真开始状态为true
    state.setSimulationRunning(true);  // 设置仿真运行状态为true
    ...
    // 把"事件-控制器"映射表传递给控制器管理线程
    controller_manager_thread.setEventDefinitions(AbortTakeoffEvents::EVENT_DEFINITIONS);

    // 初始化事件监控器
    EventMonitorThread event_monitor_thread(state, bus, AbortTakeoffEvents::EVENT_DEFINITIONS);
    ...
    // 初始化数据记录器
    FileLogger logger("abort_takeoff_log.txt");
   ...
   // ============================ 各线程的启停函数定义完成 ============================ // 
    ...
    // ================================ 按顺序启动线程 ================================ // 
    // 注意，顺序不能乱，线程的启动顺序会影响同步与实时特性
    start_simulation_control(); //第1个启动，控制仿真进程
    start_clock();              //第2个启动，控制同步性与实时性
    start_state_manager();      //第3个启动，管理共享状态空间
    start_event_monitor();      //第4个启动，开始进行事件监测
    start_controller_manager(); //第5个启动，事件驱动控制器
    start_dynamics();           //第6个启动，动力学模型运行
    start_data_recorder();      //第7个启动，记录数据
